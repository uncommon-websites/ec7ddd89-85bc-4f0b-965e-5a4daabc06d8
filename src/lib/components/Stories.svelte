<section class="py-24 bg-gradient-to-b from-white to-red-50">
	<div class="mx-auto max-w-[1400px] px-6">
		<div class="mb-12">
			<div class="mb-4 text-[10px] font-bold tracking-widest text-slate-500 uppercase">[03] Client Success</div>
			<h2 class="text-4xl font-medium tracking-tight text-gray-900">Founders who chose liquidity over forced exits</h2>
		</div>

		<div class="grid grid-cols-1 gap-8 md:grid-cols-2">
			<!-- Testimonial 1 -->
			<div class="border border-red-200 bg-red-50 p-8">
				<div class="mb-6">
					<svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" class="text-red-300">
						<path d="M3 21c3 0 7-1 7-8V5c0-1.25-.756-2.017-2-2H4c-1.25 0-2 .75-2 1.972V11c0 1.25.75 2 2 2 1 0 1 0 1 1v1c0 1-1 2-2 2s-1 .008-1 1.031V20c0 1 0 1 1 1z"/>
						<path d="M15 21c3 0 7-1 7-8V5c0-1.25-.757-2.017-2-2h-4c-1.25 0-2 .75-2 1.972V11c0 1.25.75 2 2 2h.75c0 2.25.25 4-2.75 4v3c0 1 0 1 1 1z"/>
					</svg>
				</div>
				<p class="mb-6 text-lg leading-relaxed text-gray-700">
					"I needed capital to invest in a new opportunity but didn't want to sell my shares. Fuels understood my situation immediately and structured a loan that let me keep my ownership while accessing the liquidity I needed."
				</p>
				<div class="flex items-center gap-4">
					<div class="h-12 w-12 rounded-full bg-gray-300"></div>
					<div>
						<div class="text-sm font-bold text-gray-900">Erik Lindström</div>
						<div class="text-xs text-gray-500">Founder, Nordic Tech Startup</div>
					</div>
				</div>
			</div>

			<!-- Testimonial 2 -->
			<div class="border border-slate-200 bg-slate-50 p-8">
				<div class="mb-6">
					<svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" class="text-slate-300">
						<path d="M3 21c3 0 7-1 7-8V5c0-1.25-.756-2.017-2-2H4c-1.25 0-2 .75-2 1.972V11c0 1.25.75 2 2 2 1 0 1 0 1 1v1c0 1-1 2-2 2s-1 .008-1 1.031V20c0 1 0 1 1 1z"/>
						<path d="M15 21c3 0 7-1 7-8V5c0-1.25-.757-2.017-2-2h-4c-1.25 0-2 .75-2 1.972V11c0 1.25.75 2 2 2h.75c0 2.25.25 4-2.75 4v3c0 1 0 1 1 1z"/>
					</svg>
				</div>
				<p class="mb-6 text-lg leading-relaxed text-gray-700">
					"Traditional banks couldn't understand our partnership structure. Fuels' AI-powered analysis made sense of it within days, and we had funding in our account faster than I thought possible."
				</p>
				<div class="flex items-center gap-4">
					<div class="h-12 w-12 rounded-full bg-gray-300"></div>
					<div>
						<div class="text-sm font-bold text-gray-900">Sofia Andersson</div>
						<div class="text-xs text-gray-500">Partner, Investment Firm</div>
					</div>
				</div>
			</div>
		</div>

		<!-- Stats Bar -->
		<div class="mt-16 grid grid-cols-1 gap-8 border-t border-slate-200 pt-12 md:grid-cols-3">
			<div class="text-center">
				<div class="mb-2 text-4xl font-bold text-red-600">€50M+</div>
				<div class="text-sm text-slate-600">Capital Deployed</div>
			</div>
			<div class="text-center">
				<div class="mb-2 text-4xl font-bold text-slate-900">3-5 Days</div>
				<div class="text-sm text-slate-600">Average Turnaround</div>
			</div>
			<div class="text-center">
				<div class="mb-2 text-4xl font-bold text-red-600">100%</div>
				<div class="text-sm text-slate-600">Confidential Process</div>
			</div>
		</div>
	</div>
</section>